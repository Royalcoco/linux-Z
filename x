...
ia_matrixnet/
├── bin/
│   └── iactl        ← Interface CLI (Bash + Python launcher)
├── ia_core/
│   ├── __init__.py
│   ├── hotspot.py      # start_ap, scan_clients
│   ├── sandbox.py      # init_node, send_data, push_cloud
│   ├── tokenizer.py    # registry/token generation
│   ├── crypto.py       # encryption, cloud sync
│   ├── dashboard.py    # Web + Dashboard routes
│   └── voice.py        # speech synth logic
├── templates/
│   ├── index.html
│   └── backoffice.html
├── semi_cloud/         # sandbox storage
├── server.py          # lanceur principal
└── requirements.txt
def start_hotspot(ssid,password): pass
def scan_clients(): pass  # arp-based
def init_node(): pass
def send_data(node_id,size): pass
def push_cloud(): pass  # conditional online
def tokenize(node_id,size): pass
def load_registry(): pass
def save_registry(): pass
def encrypt_and_store(node_id,size): pass
def is_online(): pass
#!/usr/bin/env bash
cmd="$1"; shift
case $cmd in
  hotspot) python3 -m ia_core.hotspot "$@" ;;
  scan) python3 -m ia_core.hotspot scan ;;
  init) python3 -m ia_core.sandbox init ;;
  send) python3 -m ia_core.sandbox send "$@" ;;
  push) python3 -m ia_core.sandbox push ;;
  dashboard) python3 server.py ;;
  *) echo "Usage: iactl {hotspot|scan|init|send|push|dashboard}";;
esac
